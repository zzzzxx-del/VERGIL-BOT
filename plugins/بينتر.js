...
let handler = async (message, { conn, text, usedPrefix, command }) => {
  if (!text) {
    return conn.reply(message.chat, "[❗] *اكتب الامر و ضع بجانبه ما تريد البحث عنه في بينتريست*", message);
  }
  ...
  for (let imageUrl of selectedImages) {
    results.push({
      ...
      'footer': proto.Message.InteractiveMessage.Footer.fromObject({
        'text': "VERGIL BOT" // ضع العلامة المائية هنا
      }),
      ...
      'nativeFlowMessage': proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
        'buttons': [{
          'name': "cta_url",
          'buttonParamsJson': "{\"display_text\":\"الرابط 📫\",\"Url\":\"https://www.pinterest.com/search/pins/?rs=typed&q=" + text + "\",\"merchant_url\":\"https://www.pinterest.com/search/pins/?rs=typed&q=" + text + "\"}"
        }]
      })
    });
  }
